...
| 38 | `patch38_profile_privilege_schutz.sql` | **Sicherheitsfix**, per Live-Test gefunden und bestätigt: `profiles_update_own` erlaubte jedem Nutzer, per direktem API-Aufruf die eigene `role` auf `'admin'` zu setzen (volle Rechte-Eskalation, da `is_admin()` nur `profiles.role` liest) sowie `character_class`/`org_id` frei zu ändern. Neuer BEFORE-UPDATE-Trigger blockiert das, außer der Ausführende ist bereits Admin — nicht destruktiv, rein additiv. **Live nach Einspielen erneut getestet: Schutz greift korrekt** |
| 39 | `patch39_profile_insert_privilege_schutz.sql` | **Sicherheitsfix, Fortsetzung von Patch 38**: die dortige Analyse deckte nur UPDATE ab — dieselbe Lücke existierte auch beim allerersten Anlegen des Profils (`profiles_insert_self`), live mit einem Wegwerf-Testaccount bestätigt (direktes INSERT mit `role:'admin'` erzeugte sofort ein fertiges Admin-Profil, ganz ohne App-Registrierungsbildschirm). Da die App offene Selbstregistrierung erlaubt, war das von jedem Internet-Besucher aus nutzbar. Neuer BEFORE-INSERT-Trigger erzwingt `role='member'` und die aktuelle Standard-`org_id` bei jeder neuen Zeile. **Live nach Einspielen erneut getestet: ein zweiter Wegwerf-Account versuchte `role:'admin'` + fremde `org_id` einzuschleusen, in der DB landete korrekt `role:'member'` + die richtige Standard-Org — Schutz bestätigt** |
