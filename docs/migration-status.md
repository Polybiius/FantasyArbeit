...
| 5 | Kanban + Kontakte gemeinsam — **Verhalten bit-identisch eingefroren** | läuft (Zustandsmaschine + gemeinsame `ContactCard` + `dnd-kit`-Board + kompletter Schreibpfad inkl. ALLER Popups — Gewonnen/Verloren, Bedarfsanalyse/Termin/Dauerbrenner — technisch fertig, Typecheck/Lint/Vitest(37)/Build grün. **Eine unabhängige 8-Agenten-Zweitmeinung (2026-09-05) fand 12 Funde, 11 davon noch am selben Tag behoben** (u.a. den echten Datenintegritäts-Bug — Wiedervorlage aus dem Gewonnen-Popup schrieb nicht `contacts.naechster_kontakt`) — Fund 7 (Vanillas `mySalesCache` nach einem React-Verkauf invalidieren) ist bewusst noch offen, siehe `src/features/kanban/README.md`. **Die von Runde 2 aufgeworfene offene Architektur-Frage (Resilienz bei fehlschlagender XP-/Trichter-Buchung) ist entschieden und gebaut (2026-09-05):** Vanilla-Verhalten übernommen (loggen + weitermachen statt Mutation abbrechen), plus Retry-mit-Backoff (`shared/lib/retry.ts`) — siehe `src/features/kanban/README.md` Abschnitt "Resilienz-Entscheidung". Noch NICHT in `App.tsx` verdrahtet/erreichbar, noch kein echter End-to-End-Test mit Login.) | Nächster Teilschritt: echter E2E-Test + Scharfschalten (Route + Vanilla-Seite ausblenden, dabei Fund 7 miterledigen), dann `contacts` serverseitig paginieren + volltextsuchen (ADR-0010), Realtime (ADR-0009, RLS-Verifikation Pflicht-Gate) |
