...
| 5 | Kanban + Kontakte gemeinsam — **Verhalten bit-identisch eingefroren** | läuft (Zustandsmaschine + gemeinsame `ContactCard` + `dnd-kit`-Board mit Lese-Vorschau fertig, **noch KEIN Schreibzugriff, noch NICHT in `App.tsx` verdrahtet/erreichbar** — siehe `src/features/kanban/README.md`) | Nächster Teilschritt: die dort dokumentierte Bridge-Lücke klären (React-Kartenzug kann Vanillas Stats-/Quest-Header aktuell nicht zum Neu-Rendern bewegen), dann der echte Schreibpfad für Übergänge ohne Popup, dann Popups (inkl. Gewonnen/Verloren), dann `contacts` serverseitig paginieren + volltextsuchen (ADR-0010), Realtime (ADR-0009, RLS-Verifikation Pflicht-Gate) |
