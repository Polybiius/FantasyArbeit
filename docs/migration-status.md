...
| 4 | App-Rahmen (Sidebar/Vollbreite, Design-Grundlage) | 🔶 läuft (2026-09-03) | Styling-Grundlage (ADR-0006) + Bridge für Level/XP/Energie + Nav-Status (ADR-0002-Nachtrag 3+4) fertig. `Sidebar.tsx`/`StatsHeader.tsx`/`navItems.ts`/`AppShell.tsx` gebaut. Isoliert per Playwright-Screenshot verifiziert (alle 3 Klassen, Pool-Modus, Nicht-Admin, Desktop/Mobil/720px-Umbruch) UND live gegen die echte App verifiziert (Nav-Bridge mit echten Admin-/Mitglied-Testkonten: aufgelöste Weiterleitungen, Hash-Korrektur, DOM/Bridge-Abgleich nach echtem Klick). `AppShell.tsx` löst den dokumentierten "nicht die ganze Seite wird genutzt"-Schmerzpunkt (`.wrap{max-width:1040px}` in `index.html`) durch ein Grid ohne äußere Breitenobergrenze. Ein echter Bug beim Bauen gefunden+behoben: `min-h-screen` ohne `content-start` blähte im mobilen 1-Spalten-Layout alle Zeilen gleichmäßig auf, wodurch Sidebar-Knöpfe sichtbar überdehnten. **Noch NICHT in `index.html` scharf geschaltet**, aktuell totes Bundle-Gewicht (nichts importiert diese Dateien). Fehlt noch: das eigentliche Scharfschalten in Produktion (eigener, mit Nutzer abgestimmter Schritt — betrifft jede Seite; geplanter Mechanismus: statisches Grid-Gerüst in `index.html`, React rendert Header+Sidebar per `createPortal()` in zwei Anker-Elemente, Vanillas `.content` bleibt unangetastet im dritten Grid-Bereich, siehe `AppShell.tsx`-Kommentar). |
