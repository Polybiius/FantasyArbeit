...
| 5 | Kanban + Kontakte gemeinsam — **Verhalten bit-identisch eingefroren** | läuft (Zustandsmaschine + gemeinsame `ContactCard` + `dnd-kit`-Board + ECHTER Schreibpfad für alle Übergänge OHNE Popup fertig, inkl. neuer Bridge-Ausnahme `notifyActionLogged()` für XP-/Quest-Sync — **noch NICHT in `App.tsx` verdrahtet/erreichbar, noch kein echter End-to-End-Test mit Login**, siehe `src/features/kanban/README.md`) | Nächster Teilschritt: Popups (Bedarfsanalyse/Termin) + der Verkaufs-Schreibpfad für Gewonnen/Verloren, dann echter E2E-Test + Scharfschalten (Route + Vanilla-Seite ausblenden), dann `contacts` serverseitig paginieren + volltextsuchen (ADR-0010), Realtime (ADR-0009, RLS-Verifikation Pflicht-Gate) |
