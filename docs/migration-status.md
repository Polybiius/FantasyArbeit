...
| 5 | Kanban + Kontakte gemeinsam — **Verhalten bit-identisch eingefroren** | läuft (Verhaltens-Vertrag als Tests fertig; ADR-0007–0011 stehen; `src/features/kanban/kanbanTransitions.ts` — reine Zustandsmaschine — fertig+getestet, noch nicht an UI angebunden) | Nächste Teilschritte: `dnd-kit` (ADR-0008) verdrahten, gemeinsame Kontakt-Karte bauen, `contacts` serverseitig paginieren + volltextsuchen (ADR-0010), Realtime für Kontakte (ADR-0009, RLS-Verifikation Pflicht-Gate), Zod an RPC-Grenzen (ADR-0011) |
